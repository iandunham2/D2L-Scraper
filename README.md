...
All CSV and HTML files generated by the utilities are stored in the `data/` directory.
The script `compile_nested_discussions.py` collates every post with all of its replies.
`data/nested_discussions.csv` includes columns `Author` and `Post` followed by
`Reply_1`, `Reply_2`, and so on depending on the number of replies.
